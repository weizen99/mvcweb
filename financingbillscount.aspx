<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="financingbillscount.aspx.vb" Inherits="Web01.financingbillscount" maintainscrollpositiononpostback="true"  %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title>financingbillcount</title>
      ...
      <nav class="navbar navbar-expand-lg navbar-dark bg-dark ptest2">
        ...
        <span class="btn btn-default">
                    <span class="auto-style1"><span class="auto-style2"><span class="auto-style9"><span class="auto-style10">(<asp:TextBox ID="TextBox11" runat="server" class="text-muted" Height="20px" Width="58px" Font-Names="Times New Roman" Font-Size="Medium" ToolTip="漲幅加總的參數(漲幅%數)或取股票代號的股要列表" BackColor="#99FF66" CssClass="text-center">50</asp:TextBox>
                          ...
                          )<asp:Button ID="Button138" runat="server" Text="單股融資券" Width="96px" BackColor="Black" ForeColor="White" Font-Names="標楷體" ToolTip="為方便查詢個股資料,會顯示上方所輸入的股票名稱,在另一個表格內" />
            ...
            runat="server" class="text-muted" Height="20px" Width="92px" Font-Size="Medium" ToolTip="請輸入行業分類或交易所分類簡稱即可" CssClass="auto-style5 text-center" BackColor="#99FF66" ForeColor="Maroon">生技</asp:TextBox>
            ...
            )<asp:Label ID="Label33" runat="server" Text=";(融資及融券比率 &gt;"></asp:Label>
                          <asp:TextBox ID="chips1" 

            runat="server" class="text-muted" Height="20px" Width="40px" Font-Size="Medium" ToolTip="" CssClass="auto-style5 text-center" BackColor="#99FF66" ForeColor="Maroon">20</asp:TextBox>

            <asp:Label ID="Label34" runat="server" Text="及券資比 &gt;"></asp:Label>
            ...
            <asp:GridView ID="GridView2" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" CellPadding="4" DataSourceID="SqlDataSource2" ForeColor="Black" BackColor="#CCCCCC" BorderColor="#999999" BorderStyle="Solid" BorderWidth="3px" CellSpacing="2">
                <Columns>
                    ...
                    <asp:BoundField DataField="資使用率黃金交叉次數" HeaderText="資使用率黃金交叉次數" SortExpression="資使用率黃金交叉次數" ReadOnly="True" />
                    <asp:BoundField DataField="券使用率" HeaderText="券使用率" SortExpression="券使用率" />
                    <asp:BoundField DataField="券使用率黃金交叉次數" HeaderText="券使用率黃金交叉次數" SortExpression="券使用率黃金交叉次數" ReadOnly="True" />
                    <asp:BoundField DataField="券資比" HeaderText="券資比" SortExpression="券資比" />
                    <asp:BoundField DataField="券資比黃金交叉次數" HeaderText="券資比黃金交叉次數" SortExpression="券資比黃金交叉次數" ReadOnly="True" />
            ...
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:stockchangedateConnectionString %>" SelectCommand="SELECT          TOP (100) PERCENT dbo.View_avg資使用率count.名稱, 
                            dbo.View_avg資使用率count.代號, dbo.View_avg資使用率count.漲跌, dbo.View_avg資使用率count.漲幅, 
                            dbo.View_avg資使用率count.成交價, dbo.View_avg資使用率count.總量, dbo.View_Financingbills.資使用率, 
                            dbo.View_avg資使用率count.資使用率黃金交叉次數, dbo.View_Financingbills.券使用率, 
                            dbo.View_avg券使用率count.券使用率黃金交叉次數, dbo.View_Financingbills.券資比, 
                            dbo.View_avg券資比count.券資比黃金交叉次數, dbo.View_avg資使用率count.產業分類, 
                            dbo.View_avg資使用率count.交易所分類, dbo.View_avg資使用率count.股本億
FROM              dbo.View_avg資使用率count INNER JOIN
                            dbo.View_avg券使用率count ON dbo.View_avg資使用率count.代號 = dbo.View_avg券使用率count.代號 INNER JOIN
                            dbo.View_avg券資比count ON dbo.View_avg資使用率count.代號 = dbo.View_avg券資比count.代號 INNER JOIN
                            dbo.View_Financingbills ON dbo.View_avg資使用率count.代號 = dbo.View_Financingbills.代號
ORDER BY   dbo.View_avg券資比count.券資比黃金交叉次數 DESC"></asp:SqlDataSource>
